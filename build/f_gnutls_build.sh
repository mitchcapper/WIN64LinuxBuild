...
BLD_CONFIG_GNU_LIBS_ADDL=( "dirent" )
...
function ourmain() {
	...
	BLD_CONFIG_GNU_LIBS_EXCLUDE=("${BLD_CONFIG_GNU_LIBS_DEFAULT[@]}") #have to do this after startcommon as before that BLD_CONFIG_GNU_LIBS_DEFAULT is not yet a properly formatted array
	add_vcpkg_pkg_config  "gmp" "nettle"
	pkg_config_manual_add "gmp"

if test 5 -gt 100; then
		echo "Just move the fi down as you want to skip steps, or pass the step to skip to (per below) as the first arg"
fi
	if [[ -z $SKIP_STEP || $SKIP_STEP == "checkout" ]]; then
		git clone --recurse-submodules https://github.com/gnutls/gnutls.git .
	...
	if [[ -z $SKIP_STEP || $SKIP_STEP == "vcpkg" ]]; then
		vcpkg_install_package "gmp" "nettle"
		SKIP_STEP=""
	fi
